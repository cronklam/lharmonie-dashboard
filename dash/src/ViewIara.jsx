...
export default function ViewIara({ data }) {
  ...
  const pending = facturas.filter(f => f['Estado']==='A pagar' || f['Medio de Pago']?.includes('A pagar'))
  const paid = facturas.filter(f => f['Estado']==='Pagado' || f['Estado']==='Pagado previamente')
        ...
        <div className={s.tableWrap}>
          <table className={s.table}>
            ...
            <tbody>
              ...
              {lista.map((f,i)=>{
                const pagada = f['Estado']==='Pagado'||f['Estado']==='Pagado previamente'
                return (
                  <tr key={i} className={pagada?s.rowPaid:''}>
                    ...
                    <td><span className={`${s.badge} ${pagada?s.badgeSuccess:s.badgeDanger}`}>{pagada?'Pagado':'Pendiente'}</span></td>
                  </tr>
                )
              })}
            </tbody>
