...
async function loadFoodCost() {
  try {
    ...
    // Header en fila 1 (index 0)
    const headers = rows[0] ? rows[0].map(h => String(h).trim()) : [];
    state.data.foodcost = rows.slice(1)
      .map(row => {
        const obj = {};
        headers.forEach((h, i) => { obj[h] = row[i] !== undefined ? String(row[i]).trim() : ''; });
        return obj;
      })
      .filter(r => r['Artículo'] && r['Artículo'] !== '');
...
function esRevisar(row) {
  // Buscar columna que diga REVISAR
  return Object.values(row).some(v => String(v).toUpperCase().includes('REVISAR'));
}

function getMargenActual(row) {
  const v = row['Margen Actual'] || row['Margen actual'] || '';
  const n = parseFloat(String(v).replace('%','').replace(',','.'));
  return isNaN(n) ? null : n;
}

function getCostoActual(row) {
  return parseNum(row['Costo Actual'] || row['Costo actual'] || row['Costo UN + IVA'] || '0');
}

function getPV(row) {
  return parseNum(row['PV Nuevo'] || row['PV'] || '0');
}
  ...
  el.innerHTML = '<div class="list-panel">' +
    rows.map(r => {
      ...
      return `${catHeader}<div class="art-list-row">
        <div class="art-list-info">
          <div class="art-list-nombre">${r['Artículo'] || '—'}</div>
