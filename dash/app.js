...
function esAPagar(f) {
  const e = String(f[COL.estado] || '').trim().toLowerCase();
  // "pagado previamente" o cualquier variante de pagado NO es a pagar
  if (esPagado(f)) return false;
  return e === 'a pagar' || e === 'pagar';
}

function esPagado(f) {
  const e = String(f[COL.estado] || '').trim().toLowerCase();
  return e.startsWith('pagado') || e.includes('previamente') || e === 'pagado';
}
