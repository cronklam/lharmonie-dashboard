...
function doLogin() {
  ...
  if (user && user.pass === p) {
    ...
    showPYLNav();
    loadAll();
    // Biometric setup available via manual button only
  } else {
...
function initBiometricLoginUI() {
  ...
  if (hasAnyBiometric()) {
    ...
    // Auto-trigger biometric login directly
...
function renderKPIInicio(facturas) {
  ...
  // Calculate change (capped at 999% to avoid absurd display)
  let change = prevMonthTotal > 0 ? ((currentMonthTotal - prevMonthTotal) / prevMonthTotal) * 100 : 0;
  ...
  el.innerHTML = `
    <div class="kpi-card kpi-card-main kpi-card-unified">
      <div class="kpi-label">${periodoLabel}</div>
      <div class="kpi-value">${fmtMoney(currentMonthTotal)}</div>
      <div class="kpi-sub-row">
        <span class="kpi-promedio">Prom. diario: ${fmtMoney(dailyAvg)}</span>
        ${changeHTML}
      </div>
    </div>
  `;
...
function showPYLNav() {
  ...
  if (navPyl && state.user && state.user.role === 'Administrador') {
    navPyl.style.display = '';
  }
}
