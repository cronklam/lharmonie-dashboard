...
async function confirmarPago() {
  ...
  // ── 2. Sincronizar con el Sheet en segundo plano ──
  try {
    ...
    if (res.ok && data.ok) {
      console.log('✅ Sheet actualizado:', data.message);
    } else {
      console.warn('⚠️ Worker no encontró la fila:', data);
      showToast('⚠️ Guardado localmente, verificar Sheet');
    }
  } catch (err) {
    console.warn('⚠️ Sin conexión al worker:', err.message);
    showToast('⚠️ Sin conexión, verificar Sheet');
  }
}
