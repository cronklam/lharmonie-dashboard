...
function renderAll() {
  ...
  if (hoyFacts.length) {
    ...
    hoyList.innerHTML = hoyOrdered.map((f, i) => {
      ...
      return `<div class="factura-card ${pagado ? 'pagada' : 'pendiente'}" onclick="abrirDetalle('${fJson}')">
        <div class="factura-main">
          <div class="factura-info">
            <div class="factura-proveedor">${f[COL.proveedor] || '—'}</div>
            <div class="factura-meta">Nº ${f[COL.nroFac] || '—'} · ${f[COL.fecha] || '—'}<br>${f[COL.local] || ''}${f[COL.cajero] ? '<br>📋 ' + f[COL.cajero] : ''}</div>
          </div>
        ...
        <div class="factura-footer">
          ...
          ${!pagado ? `<button class="btn-pagar" onclick="event.stopPropagation();abrirModal(${i},'hoy')">Marcar pagada</button>` : ''}
        </div>
      </div>`;
    }).join('');
  } else {
...
function renderAPagar(pendientes) {
  ...
  el.innerHTML = filtradas.map((f, i) => {
    const fJson = JSON.stringify(f).replace(/"/g,'&quot;');
    return `<div class="factura-card pendiente" onclick="abrirDetalle('${fJson}')">
      <div class="factura-main">
        <div class="factura-info">
          <div class="factura-proveedor">${f[COL.proveedor] || '—'}</div>
      ...
      <div class="factura-footer">
        ...
        <button class="btn-pagar" onclick="event.stopPropagation();abrirModal(${i},'apagar')">Marcar pagada</button>
      </div>
    </div>`;
  }).join('');
}
...
function renderHistorial(pagadas) {
  ...
  el.innerHTML = resumenHTML + filtradas.slice(0, 60).map((f, i) => {
    const fJson = JSON.stringify(f).replace(/"/g,'&quot;');
    return `<div class="historial-card" onclick="abrirDetalle('${fJson}')">
...
/* ---- DETALLE FACTURA ---- */
function abrirDetalle(fStr) {
  let f;
  if (typeof fStr === 'string') {
    try { f = JSON.parse(fStr.replace(/&quot;/g, '"')); } catch(e) { return; }
  } else { f = fStr; }
    ...
    <div style="margin-top:14px;border-top:1px solid var(--border);padding-top:10px;">
      ...
      ${itemsFactura.map(a => {
        ...
        return `<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border);gap:8px;align-items:center;">
          <span style="font-size:12px;color:var(--muted);">${nombre}${unidad ? ' · '+unidad : ''}</span>
          ${precio > 0 ? `<span style="font-size:12px;font-weight:500;color:var(--muted);white-space:nowrap;">${fmtMoney(precio)}</span>` : ''}
        </div>`;
      }).join('')}
    </div>` : '';
...
async function confirmarPagoDirecto() {
  ...
  state.data.facturas = state.data.facturas.map(fac =>
    fac[COL.nroFac] === f[COL.nroFac] && fac[COL.proveedor] === f[COL.proveedor]
      ? { ...fac, [COL.estado]: '✅ Pagado', [COL.fechaPago]: fechaPago } : fac);
  renderAll();
