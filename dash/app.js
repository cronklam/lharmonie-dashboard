...
let state = {
  ...
  _pagadasActuales: [],
  _hoyActuales: [],
  _rawRows: [], // filas crudas del Sheet para buscar fila exacta
};
...
function rowsToObjects(rows) {
  ...
  return rows.slice(hi + 1)
    .map((row, rowIdx) => {
      ...
      obj._sheetRow = hi + 2 + rowIdx; // número de fila real en el Sheet (1-indexed)
      return obj;
    })
...
async function loadAll() {
  ...
  try {
    ...
    state._rawRows = fRows; // guardar filas crudas
...
async function confirmarPago() {
  ...
  const filaExacta = f._sheetRow || null; // fila real en el Sheet
  ...
  try {
    const res = await fetch(`${CONFIG.WORKER_URL}/update-estado`, {
      ...
      body: JSON.stringify({ nroFactura, proveedor, fechaPago, fecha, filaExacta }),
    });
    ...
    // El worker respondió pero dijo que no encontró la fila — mostrar error claro
    console.warn('Worker respondió pero no encontró la fila:', data);
    showToast('⚠️ No se pudo guardar en el servidor');

  } catch (err) {
    console.error('Error al contactar worker:', err);
    showToast('⚠️ Sin conexión al servidor');
  }

  // Fallback optimista — al menos actualizar localmente
  state.data.facturas = state.data.facturas.map(fac =>
    fac._sheetRow === f._sheetRow
...
async function desmarcarPagada(idx) {
  ...
  try {
    await fetch(`${CONFIG.WORKER_URL}/update-estado`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-api-secret': CONFIG.API_SECRET },
      body: JSON.stringify({ nroFactura: f[COL.nroFac], proveedor: f[COL.proveedor], estado: 'A pagar', fechaPago: '', filaExacta: f._sheetRow }),
    });
  } catch(_) {}
}
