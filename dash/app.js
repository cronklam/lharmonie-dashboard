...
function getAlias(proveedor) {
  // console.log('getAlias called with:', proveedor, '-> result:', aliasMap[(proveedor||'').toUpperCase().trim()]);
  if (!proveedor) return '';
...
async function cargarAliases() {
  try {
    ...
    rows.slice(2).forEach(row => { // skip header rows
      const razonSocial   = (row[0] || '').trim();
      const nombreFantasia = (row[1] || '').trim(); // Col B
      if (razonSocial && nombreFantasia) {
        aliasMap[razonSocial.toUpperCase().trim()] = nombreFantasia;
        // Also index by first word for partial matching
        const firstWord = razonSocial.toUpperCase().trim().split(' ')[0];
        if (firstWord.length > 3) aliasMap[firstWord] = nombreFantasia;
      }
    });
    console.log('Aliases cargados:', Object.keys(aliasMap).length, aliasMap);
  ...
  el.innerHTML = resumenHTML + [...pagadas].reverse().slice(0, 40).map((f, i) => `
    <div class="historial-card">
      ...
      <div style="text-align:right">
        ...
        ${getAlias(f[COL.proveedor]) ? `<div style="font-size:10px;color:var(--muted);margin-top:1px;">${getAlias(f[COL.proveedor])}</div>` : ''}
...
function renderAlertaDuplicados(dupes) {
  ...
  el.style.cssText = 'display:flex;align-items:center;gap:8px;background:#C4A882;border-radius:0;padding:8px 16px;margin:0;color:#1A0F08;';
  el.innerHTML = `<div style="font-size:12px"><span style="font-weight:600;color:#1A0F08;">⚠️ Posibles duplicados:</span> ${dupes.map(d => `<span style="color:rgba(26,15,8,0.7)">${d.prov} Nº${d.nro}</span>`).join(', ')}</div>`;
  return;
