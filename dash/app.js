...
function renderAlertaDuplicados(dupes) {
  ...
  if (!el) {
    ...
    const inicio = document.getElementById('page-inicio');
    inicio.insertBefore(el, inicio.firstChild);
  }
...
async function loadFoodCost() {
  try {
    ...
    // Fila 1 = headers, fila 2 = "Categoría" label, datos desde fila 3
    // La categoría está en col A cuando aparece sola, y se propaga hacia abajo
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // Si col A tiene solo un valor y col B está vacía, es una categoría
      ...
      // Fila de datos válida — necesita artículo
      const articulo = obj['Articulo'] || obj['Artículo'] || colC || '';
      if (!articulo || articulo === 'Artículo' || articulo === 'Articulo') continue;

      obj['Categoría'] = currentCat;
      obj['_articulo'] = articulo;
      items.push(obj);
    }
...
function esRevisar(row) {
  // Columnas específicas que indican REVISAR (no el header)
  const checkCols = ['Costo 25%', 'Costo 30%', 'Costo 20%', 'Margen Actual'];
  for (const col of checkCols) {
    if (String(row[col] || '').toUpperCase() === 'REVISAR') return true;
  }
  // También buscar en cualquier columna que no sea header ni artículo
  return Object.entries(row)
    .filter(([k]) => !['Artículo','Articulo','_articulo','Código','Categoría'].includes(k))
    .some(([,v]) => String(v).toUpperCase() === 'REVISAR');
}

function getMargenActual(row) {
  const v = row['Margen Actual'] || row['Margen actual'] || row['Margen Real'] || '';
  const s = String(v).replace('%','').replace(',','.').trim();
  const n = parseFloat(s);
  return isNaN(n) ? null : n;
}

function getCostoActual(row) {
  return parseNum(row['Costo Actual'] || row['Costo actual'] || row['Costo UN + IVA'] || row['Costo UN'] || '0');
}

function getPV(row) {
  return parseNum(row['PV Nuevo'] || row['PV Martín'] || row['PV'] || row['PV '] || '0');
}
...
function renderFoodCost() {
  ...
  if (kpiEl && state.data.foodcost) {
    ...
    kpiEl.innerHTML = `
      ...
      <div class="kpi-prov-card">
        <div class="kpi-prov-left"><div class="kpi-prov-label">Margen promedio</div><div class="kpi-prov-sub">${topCatRevisar ? `más crítico: ${topCatRevisar[0]}` : 'del menú'}</div></div>
  ...
  // Ordenar: REVISAR primero
  rows.sort((a, b) => {
    const ar = esRevisar(a) ? 0 : 1;
    const br = esRevisar(b) ? 0 : 1;
    return ar - br;
  });
  ...
  el.innerHTML = '<div class="list-panel">' +
    rows.map(r => {
      ...
      return `${catHeader}<div class="art-list-row">
        <div class="art-list-info">
          <div class="art-list-nombre">${r['_articulo'] || r['Artículo'] || r['Articulo'] || '—'}</div>
          <div class="art-list-meta">${costo > 0 ? 'Costo: ' + fmtMoney(costo) : ''}${pv > 0 ? '  ·  PV: ' + fmtMoney(pv) : ''}</div>
        </div>
        <div style="text-align:right;flex-shrink:0;">
          <div style="font-size:16px;font-weight:700;color:${margenColor};line-height:1;">${margen !== null ? margen + '%' : '—'}</div>
