...
async function cargarAliases() {
  try {
    ...
    console.log('Aliases cargados:', Object.keys(aliasMap).length);
...
function renderAlertaDuplicados(dupes) {
  ...
  el.style.cssText = 'display:flex;align-items:center;gap:8px;background:#C4A882;border-radius:0;padding:8px 16px;margin:0;';
  el.innerHTML = `<span style="font-size:13px">⚠️</span><div style="font-size:12px"><span style="font-weight:600;color:#8B6340;">Posibles duplicados:</span> ${dupes.map(d => `<span style="color:#9C9187">${d.prov} Nº${d.nro}</span>`).join(', ')}</div>`;
  return;
