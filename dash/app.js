...
function renderProveedores() {
  const el = document.getElementById('provList2');
  if (!el) return;
  const q  = (document.getElementById('provSearch') || {}).value || '';
  let rows = filterRows(state.data.proveedores || [], q).slice(0, 50);
  if (!rows.length) {
    el.innerHTML = '<div class="empty-state"><div class="empty-icon">🏢</div>Sin resultados</div>';
    return;
  }
  ...
  el.innerHTML = rows.map(r =>
    '<div class="factura-card pendiente" style="border-left:4px solid var(--brown-light)">' +
    '<div class="factura-main"><div class="factura-info">' +
    '<div class="factura-proveedor">' + (r[keys[0]] || '—') + '</div>' +
    '<div class="factura-meta">' + keys.slice(1).map(k => r[k] ? k + ': ' + r[k] : '').filter(Boolean).join(' · ') + '</div>' +
    '</div></div></div>'
  ).join('');
}
...
function renderArticulos() {
  ...
  let rows = filterRows(state.data.articulos || [], q).slice(0, 50);
  if (!rows.length) {
    el.innerHTML = '<div class="empty-state"><div class="empty-icon">📦</div>Sin resultados</div>';
    return;
  }
  ...
  const kProv   = keys.find(k => /proveedor/i.test(k)) || keys[1];
  const kPrecio = keys.find(k => /precio|unit/i.test(k));
  const kFecha  = keys.find(k => /fecha/i.test(k));
  el.innerHTML = rows.map(r =>
    '<div class="factura-card pendiente" style="border-left:4px solid var(--brown-light)">' +
    '<div class="factura-main"><div class="factura-info">' +
    '<div class="factura-proveedor">' + (r[kNombre] || '—') + '</div>' +
    '<div class="factura-meta">' + (kProv && r[kProv] ? r[kProv] + ' · ' : '') + (kFecha && r[kFecha] ? r[kFecha] : '') + '</div>' +
    '</div>' +
    (kPrecio && r[kPrecio] ? '<div class="factura-monto" style="font-size:18px;font-weight:600">' + fmtMoney(parseNum(r[kPrecio])) + '</div>' : '') +
    '</div></div>'
  ).join('');
}
