...
function renderChartsProveedores() {
  ...
  kpiEl.innerHTML =
    `<div class="kpi-prov-card">
      <div class="kpi-prov-label">Total gastado</div>
      <div class="kpi-prov-value">${fmtMoney(totalGastado)}</div>
      <div class="kpi-prov-sub">${cantProveedores} proveedores</div>
    </div>
    <div class="kpi-prov-card">
      <div class="kpi-prov-label">Mayor categoría</div>
      <div class="kpi-prov-value" style="font-size:13px">${topCat ? topCat[0] : '—'}</div>
      <div class="kpi-prov-sub">${topCat ? fmtMoney(topCat[1]) : ''}</div>
    </div>
    <div class="kpi-prov-card">
      <div class="kpi-prov-label">Top proveedor</div>
      <div class="kpi-prov-value" style="font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${topProv ? topProv[0].split(' ')[0] : '—'}</div>
      <div class="kpi-prov-sub">${topProv ? fmtMoney(topProv[1]) : ''}</div>
    </div>`;
