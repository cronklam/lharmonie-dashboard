...
async function loadFoodCost() {
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // Saltar headers repetidos o label "Categoria"
      if (colA === 'Categoria' || colA === 'Categoría' || colC === 'Articulo' || colC === 'Artículo') continue;

      // Fila de categoría: col A tiene nombre, col C vacía, col B no es número válido
      if (colA && !colC) {
        const bNum = parseFloat(colB.replace(/[^0-9.]/g,''));
        if (!colB || isNaN(bNum)) { currentCat = colA; continue; }
      }
      ...
      const margenSht = pct(g(row, I.margenActual));
      const margenIdeal = pct(g(row, I.margenIdeal)) || 25;
      const margenCalc  = pvVal > 0 && costoIVA > 0 ? Math.round((pvVal - costoIVA) / pvVal * 100) : null;
      const margenFinal = (margenSht !== null && margenSht !== 0) ? margenSht : margenCalc;

      const rev25 = g(row, I.costo25).toUpperCase() === 'REVISAR' || g(row, I.precioSug25).toUpperCase().includes('REVISAR');
      const rev30 = g(row, I.costo30).toUpperCase() === 'REVISAR' || g(row, I.precioSug30).toUpperCase().includes('REVISAR');
      ...
      items.push({
        ...
        _costoIVA:    costoIVA,
        _pv:          pvVal,
        _margenActual: margenFinal,
        _margenIdeal:  margenIdeal,
        _revisar:     rev25 || rev30 || falta || (margenFinal !== null && margenFinal < margenIdeal - 3),
        _faltaCosto:  falta,
      });
    }
