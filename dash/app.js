...
function renderAll() {
  ...
  const transf   = pendientes.filter(f => {
    const m = String(f[COL.medioPago] || f[COL.estado] || '').toLowerCase();
    return m.includes('transf') || m.includes('banco') || m.includes('transferencia');
  });
  const efectivo = pendientes.filter(f => {
    const m = String(f[COL.medioPago] || f[COL.estado] || '').toLowerCase();
    return m.includes('efectivo') || m.includes('cash');
  });
  const totalTransf   = transf.reduce((s,f) => s + parseNum(f[COL.total]), 0);
  const totalEfectivo = efectivo.reduce((s,f) => s + parseNum(f[COL.total]), 0);
  ...
  document.getElementById('statTransf').textContent     = fmtMoney(totalTransf);
  document.getElementById('statTransfCount').textContent= `${transf.length} factura${transf.length !== 1 ? 's' : ''}`;
  document.getElementById('statEfectivo').textContent   = fmtMoney(totalEfectivo);
  document.getElementById('statEfectivoCount').textContent = `${efectivo.length} factura${efectivo.length !== 1 ? 's' : ''}`;
...
function renderChartsProveedores() {
  ...
  // 2. Top proveedores por gasto total (bar horizontal)
  const provMap = {};
  facturas.forEach(f => {
