...
function renderAll() {
  ...
  if (hoyFacts.length) {
    ...
    hoyList.innerHTML = hoyOrdered.map((f, i) => {
      ...
      return `<div class="factura-card ${cardClass}" onclick="abrirDetalle('${fJson}')">
        ...
        <div class="factura-footer">
          ${bistro
            ? '<span class="factura-estado bistrosoft">🤖 Cargado por Bistrosoft</span>'
...
function renderAPagar(pendientes) {
  ...
  el.innerHTML = filtradas.map((f, i) => {
    const bistro = esBistrosoft(f);
    const cardClass = bistro ? 'bistrosoft' : 'pendiente';
    const fJson = JSON.stringify(f).replace(/"/g,'&quot;');
    return `<div class="factura-card ${cardClass}" onclick="abrirDetalle('${fJson}')">
      <div class="factura-main">
        <div class="factura-info">
