...
async function loadFoodCost() {
  try {
    ...
    // Índices fijos basados en la estructura real del Sheet (sin contar columnas ocultas)
    ...
    const get = (row, idx) => row[idx] !== undefined ? String(row[idx]).trim() : '';

    let currentCat = '';
    const items = [];
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i];
      const colA = get(row, IDX.categoria);
      ...
      // Necesita artículo para ser válida
      if (!colC || colC === 'Artículo' || colC === 'Articulo') continue;

      const margenActualRaw = get(row, IDX.margenActual);
      const margenIdealRaw  = get(row, IDX.margenIdeal);
      const revisar25 = get(row, IDX.costo25).toUpperCase() === 'REVISAR' || get(row, IDX.precioSug25).toUpperCase().includes('REVISAR');
      const revisar30 = get(row, IDX.costo30).toUpperCase() === 'REVISAR' || get(row, IDX.precioSug30).toUpperCase().includes('REVISAR');
      const faltaCosto = get(row, IDX.costo25).toUpperCase().includes('FALTA') || get(row, IDX.costo30).toUpperCase().includes('FALTA');

      items.push({
        _articulo:    colC,
        _codigo:      colB,
        'Categoría':  currentCat,
        _costoUN:     get(row, IDX.costoUN),
        _costoIVA:    get(row, IDX.costoUNiva),
        _pv:          get(row, IDX.pv),
        _pvNuevo:     get(row, IDX.pvNuevo),
        _pvMartin:    get(row, IDX.pvMartin),
        _margenActual: margenActualRaw,
        _margenIdeal:  margenIdealRaw,
        _costoActual:  get(row, IDX.costoActual),
        _revisar:     revisar25 || revisar30,
        _faltaCosto:  faltaCosto,
      });
    }
